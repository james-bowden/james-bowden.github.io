...
2. Given $L$ and $A$, we can write the design space as $X := A^L$, i.e., all proteins of length $L$. It follows that there are $\mid A\mid^L=20^L$ possible proteins to choose from.
...
<li>{{ site.indent }}randomly, as a pre-trained model, as uniform on a set of designs, ...</li>
