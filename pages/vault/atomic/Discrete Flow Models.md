...
date modified: Wednesday, January 29th 2025, 10:48:10 pm
tags:
  - atomic
  - discrete_data
  - CTMC
...
~~Difference btwn $$Q, R$$ ? — why does it matter? I think they use $$R$$ to be analogous to $$u_t(x)$$ vector field in [Flow Matching](dne.md)? Wrong.~~
...
![Argmax flows, multinomial diffusion#[Argmax Flows and Multinomial Diffusion Learning Categorical Distributions](https //proceedings.neurips.cc/paper/2021/hash/67d96d458abdef21792e6d8e590244e7-Abstract.html)](dne.md)
![Discrete diffusion models#[Structured Denoising Diffusion Models in Discrete State-Spaces](https //proceedings.neurips.cc/paper/2021/hash/958c530554f78bcd8e97125b70e6973d-Abstract.html)](dne.md)

## Previous work on continuous time diffusion / flow modeling

![Flow Matching#[Flow Matching for Generative Modeling](https //arxiv.org/abs/2210.02747)](dne.md)

![Flow Matching#^b833ff](dne.md)

## CTMC setup
![Continuous Time Markov Chain](dne.md)

# Discrete Flow Models (p 3-?)
![image](img/Pasted image 20250125175920.png)
Map from [Flow Matching](dne.md) to DFM (present).

Procedure: ![image](img/Pasted image 20250125191524.png)

## DFM Definition

**Key idea:** parameterize a continuous-time *probability flow* (a la [Continuous Time Markov Chain](dne.md); [Flow Matching](dne.md)); this flow will operate on probability mass vectors (also continuous-valued) that represent transition probabilities for a discrete data distribution. In this way, we can have a discrete data generative model that’s parameterized with all continuous underlying probabilities and flows[^4].

Define conditional flow, as in [Flow Matching](dne.md):
...
From [Continuous Time Markov Chain#Kolmogorov equation](dne.md), to define $$p_t(x_t)$$ we need $$R_t(x_t, j)$$.
...
Using insights from [Flow Matching#Conditional flow matching](dne.md), we can define the rate matrix as datapoint-conditional:
...
How do we know that $$R_t^*$$ generates $$p_{t|1}(x_t|x_1)$$? We can plug $$R_t^*$$ into the [Continuous Time Markov Chain#Kolmogorov equation](dne.md). Relies on assuming that any states with zero probability mass also have time-derivative of 0 (cannot recover from 0 mass state)[^9]. ^4b1ac6

We can substitute in the forms for noising processes to get![image](img/Pasted image 20250125185616.png) ^361e45
